...
main() {
	...
	if [[ "$(id -u)" != '0' ]]
	then
		echo -e "\e[31mError! script must be ran as root"
		exit 1
	fi
	...
	local kube_version="$(curl -sSL https://dl.k8s.io/releases/stable.txt | sed 's/\([0-9]*\)\.\([0-9]*\)\.[0-9]*/v\1.\2/')" # get kubernetes latest version number
	local version="${kube_version#v}" # remove the first v from the kube_version variable's content
